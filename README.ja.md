...
Samsung Galaxy Z Foldの開閉角度に合わせて、前面と内側の画面をすりガラス越しにつなぐ実験アプリです。開閉中はアプリの像をその場に残すように視差とぼかしを加え、もう一方の画面へ引き継ぎます。ホームアプリを変更せず、通常のアプリで使えます。

[0.1.21をダウンロード](https://github.com/bunkaich/Folduo/releases/tag/v0.1.21)

## 必要な環境

- **Samsung Galaxy Z Foldシリーズ対応**（Fold4以降、両画面同時点灯対応機種）。対応しない機種では画面制御を開始しません。
- 確認した環境：Galaxy Z Fold7 / Android 16 / One UI 8.5。
...
アプリ上部の **言語 / Language** で、**日本語**・**English**・**简体中文**・**端末の設定に合わせる**を選べます。選択は保存され、Androidのアプリ別言語設定とも連動します。初期状態では、日本語の端末は日本語、それ以外は英語で表示します。
...
確認したSamsung Fold端末では、Samsungが内側からの新規アプリ起動を前面へ振り替えていました。Folduoのホームは選んだアプリだけを内側へ移し、ホームへの復帰も揃えます。他のランチャーの同症状を修正するものではありません。
